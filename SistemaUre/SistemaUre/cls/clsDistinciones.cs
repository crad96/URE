...
{
    public class clsDistinciones
    { private clsConec conec = null;

    public clsDistinciones(string host, string dataBase, string user, string pass)
       
        {
            conec = new clsConec(host, dataBase, user, pass);
        }

    public clsDistinciones()
        {
            conec = new clsConec();
        }
    ...
    {
        clsConec cnn = new clsConec();
        SqlCommand cmd = new SqlCommand("exec sp_insert_distincion_Reci '" + entidad + "',  '" + distincion+ "'  , '" + fecha  + "',  '" + dni_persona + "' ", cnn.loadConeccion());
    ...
    {
        string query = "Select codi,entidad, distincion, fecha from v_distincion_Reci where dni_persona='" + DNI + "'";
    ...
    {
        clsConec cnn = new clsConec();
        SqlCommand cmd = new SqlCommand("exec sp_delete_distincion_Reci " + codi + "", cnn.loadConeccion());
        return cmd.ExecuteScalar();
    }
